...
import type { Block, LLMResponse, Message, StopReason, ToolDef } from "../api/types.js";
	...
	async send(messages: Message[], tools: ToolDef[]): Promise<LLMResponse> {
		try {
			const resp = await this.client.chat.completions.create({
				...
				messages: [{ role: "system", content: this.system }, ...this.toOpenAIMessages(messages)],
			...
			const stopReason: StopReason = content.some((block) => block.type === "tool_use")
				? "tool_use"

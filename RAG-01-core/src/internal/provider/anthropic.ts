...
// Two private methods handle the bidirectional type translation:
//   toAnthropicMessages()  harness Message[] → SDK MessageParam[]
//   toAnthropicTools()     harness ToolDef[] → SDK Tool[]
//   fromAnthropicContent() SDK ContentBlock[] → harness Block[]
...
export class AnthropicProvider implements Provider {
	...
	// §06: The system prompt is a top-level API field, NOT part of messages[].
	// Provider owns it so the loop never has to handle it specially.
	...
	async send(messages: Message[], tools: ToolDef[]): Promise<LLMResponse> {
		try {
			...
			if (err instanceof Anthropic.APIError) {
				throw new ProviderError(err.message, err.status, err.status >= 500 || err.status === 429);
			}
			throw err;
		}
	}
	...
	private toAnthropicTools(tools: ToolDef[]): Anthropic.Tool[] {
		// §17 Sort by name for stable ordering — random ordering invalidates
		// the prompt cache prefix on every call (applied in harness-03).
		return [...tools]
	 ...
	 * @returns Harness-format blocks consumed by the agent loop.
	 */
	private fromAnthropicContent(content: Anthropic.ContentBlock[]): Block[] {
		...
		for (const b of content) {
			...
			// are internal SDK blocks that are not part of the harness protocol — skip.
		}
		return blocks;
	}
}

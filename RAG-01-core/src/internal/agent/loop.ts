...
import type { Block, LLMResponse, Message } from "../api/types.js";
import type { Provider } from "../provider/index.js";
import { ProviderError } from "../provider/index.js";
import type { Registry } from "../tool/registry.js";
import { printError, printText, printToolCall, printToolResult } from "../ui/output.js";
	...
	while (true) {
		...
		for (const block of response.content) {
			...
			if (block.type === "tool_use") {
				...
				const { result, isError } = await registry.execute(block.toolName, block.toolInput);

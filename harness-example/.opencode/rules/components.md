...
- Use `className` strings from `@/components/tokens` (`cls.*`) for all UI elements — buttons, cards, badges, metric tiles, form fields.
- CSS custom properties from `globals.css` (`var(--color-primary)`, `var(--space-xl)`, etc.) are available everywhere.
- No inline `style=` props unless Foundation has no matching class AND you use only token variables (`var(--space-*)`, `var(--color-*)`).
- No external CSS libraries, no Tailwind, no CSS modules within Section folders.
- Never set `font-family` or `font-size` directly — use the `Heading` primitive for all headed text.
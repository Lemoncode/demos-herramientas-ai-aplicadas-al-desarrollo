...
After `fleet/<section_id>` is checked out, every `Write` / `Edit` outside `src/components/<section_id>/` is hard-blocked by `.opencode/hooks/section-ownership.sh`. This is intentional. If the hook ever blocks you, the violation is real — fix the path, do not work around it.
...
**Styling — use Foundation, do not freestyle:**
- The Foundation generates a full set of CSS classes in `globals.css`. Use them via `className`; never use inline `style=` props.
- Import class-name constants from `@/components/tokens` (`cls.card`, `cls.primaryBtn`, `cls.metricTile`, etc.).
- Section-specific layout (grid columns, gap, padding) may use CSS custom properties (`var(--space-xl)`) from `globals.css`.
- No local `.module.css` files. No `<style>` tags. No `@emotion` or similar.
- If Foundation has no class for an element you need (e.g. an accordion panel), write a minimal inline `style` object using only token variables: `{ padding: 'var(--space-md)', color: 'var(--color-text)' }`.

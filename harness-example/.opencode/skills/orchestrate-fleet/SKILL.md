...
If branch IS `main`, halt with: `"orchestrate-fleet must never run on main. Create a demo branch (e.g. git checkout -b demo-<date>) and re-run /goal."`
...
Invoke the `frontend-design` skill with this full brief:

**Purpose:** Marketing landing page for JivaEnergy — a Spanish manufacturer of AC/DC EV chargers.
Audience: commercial fleet managers, B2B distributors, public-sector procurement officers.
The page must communicate precision, technical credibility, and European manufacturing authority.

**Aesthetic direction:** Editorial Energy — magazine-editorial meets industrial precision.
Think: Dezeen, Wallpaper*, European industrial B2B. Clean but not sterile. Confident but not loud.
NOT generic SaaS. NOT purple gradients. NOT Inter on white.

**Palette (locked):**
- Primary: #1a4731 (deep forest green — authority, sustainability, precision)
- Background: #f5f0e8 (warm bone — premium paper stock feel)
- Accent: #e8d44d (sulfur yellow — sharp technical highlight)
- Text: #1a1a1a

**Typography mandate:**
- Headings: a distinctive editorial serif (e.g. Playfair Display, Cormorant Garamond, or DM Serif Display) loaded via `@import` in globals.css. Must feel authoritative, not decorative.
- Body: a clean humanist sans that pairs well (e.g. DM Sans, Outfit) — NOT Inter, NOT Roboto.

**What makes this unforgettable:** The tension between the warm paper-stock background and sharp sulfur yellow accents — like a beautifully typeset engineering spec sheet. Section transitions should feel deliberate. Typography must be the hero.

**Required output (beyond tokens + primitives):**
1. `globals.css` — Google Fonts `@import` + CSS custom properties + a rich set of named utility classes: `.btn-primary`, `.btn-secondary`, `.card`, `.badge-residential`, `.badge-commercial`, `.metric-tile`, `.form-field`, `.grid-4col`, `.grid-2col` — that Section Workers use directly via `className`.
2. `tokens.ts` — export the CSS class name strings as typed constants (`cls.card`, `cls.primaryBtn`, `cls.metricTile`, etc.) so workers import and reference them without memorising class names.
3. Foundation primitives — must carry real `className`-based styling using the token classes, not bare HTML.

**Constraints:** Next.js 15 App Router, React 19 server components, no external UI libraries, no Tailwind.

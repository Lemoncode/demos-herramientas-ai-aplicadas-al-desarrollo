...
permission:
  ...
  bash:
    ...
    "npm run lint*": allow
    "git add src/components/directory/*": allow
    "git commit*": allow
    "git status*": allow
    "git branch*": allow
    "git rev-parse*": allow
...
**First step: invoke the `fix-ticket` skill.** Do not write a single file until you have read it. Then follow every step in order.
